...
class stack::apps::redmine::install($path='/opt/redmine', $owner='redmine') {
  ...
  $redmine_source =
    'http://rubyforge.org/frs/download.php/76448/redmine-2.1.0.tar.gz'
  ...
  exec {'bundle_redmine':
    ...
    command     => "bash -c '${rvmsource}; \
      redmine_bundle install --without development test mysql sqlite'",
